...
public class CustomRidePathOverlay {
    ...
    public CustomRidePathOverlay(AMap aMap, RidePath path, LatLonPoint startPoint, LatLonPoint endPoint, List<LatLonPoint> passPoints) {
        this.mAMap = aMap;
        this.mRidePath = path;
    ...
    public void addRouteToMap(int color) {
        ...
        if (mAMap != null && mRidePath != null && mRouteWidth != 0) {
...
//                addStationMarkers(step, MapUtil.getInstance().convertToLatLng(points.get(0)));
    ...
    private void updatePathColor() {
        if (mAMap != null && mLatLngPaths != null && mLatLngPaths.size() > 0) {
            int j = 0;
            ...
            for (int i = 0; i < mLatLngPaths.size(); i++) {
