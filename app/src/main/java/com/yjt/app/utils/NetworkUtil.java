...
import com.yjt.app.constant.Net;
...
public class NetworkUtil {
    ...
    public boolean isConnectingToInternet() {
        ConnectivityManager connectivity = (ConnectivityManager) BaseApplication.getInstance()
                .getSystemService(BaseApplication.getInstance().CONNECTIVITY_SERVICE);
    ...
    public String getNetworkType() {
        String      type = null;
        NetworkInfo info = ((ConnectivityManager) (BaseApplication.getInstance().getSystemService(Context.CONNECTIVITY_SERVICE))).getActiveNetworkInfo();
        if (info != null && info.isConnected()) {
            if (info.getType() == ConnectivityManager.TYPE_WIFI) {
                type = Net.WIFI.getContent();
                ...
                switch (networkType) {
                    ...
                    case TelephonyManager.NETWORK_TYPE_HSPAP:  //api<13 : replace by 15
                        type = Net.GGG.getContent();
                        break;
                    case TelephonyManager.NETWORK_TYPE_LTE:    //api<11 : replace by 13
                        type = Net.GGGG.getContent();
                        break;
                    default:
                        if (info.getSubtypeName().equalsIgnoreCase(Net.TD_SCDMA.getContent())
                                || info.getSubtypeName().equalsIgnoreCase(Net.WCDMA.getContent())
                                || info.getSubtypeName().equalsIgnoreCase(Net.CDMA2000.getContent())) {
                            type = "3G";
                        } else {
    ...
    public boolean download(String url, File file) {
        ...
        try {
            ...
            connection.connect();
            LogUtil.print("----size:" + connection.getContentLength());
